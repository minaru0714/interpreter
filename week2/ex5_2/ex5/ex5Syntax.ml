...
let rec eval:env -> expr -> value = fun env expr -> match expr with 
  ...
  | EApp (e1,e2) ->
    ...
    | VRecFunand (fn, xn, oenv) -> 
        let (fi, xi, ei) = List.nth xn (fn-1) in
        let env' = (xi, eval env e2) :: (List.mapi (fun idx (f, _, _) -> (f, VRecFunand (idx+1, xn, oenv))) xn) @ oenv in
        eval env' ei 
  ...
  | ERecFunand (fs, e) ->
      let env' = List.fold_left (fun acc_env (f, x, e') ->
        (f, VRecFun(f, x, e', acc_env)) :: acc_env) env fs
      in
      eval env' e
